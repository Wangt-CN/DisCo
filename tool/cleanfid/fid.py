...
from tool.cleanfid.utils import *
from tool.cleanfid.features import build_feature_extractor, get_reference_statistics
from tool.cleanfid.resize import *
...
def get_files_features(l_files, model=None, num_workers=12,
    ...
    # define the models if it is not specified
    if model is None:
...
"""
Compute the FID stats from a generator models
"""
def get_model_features(G, model, mode="clean", z_dim=512,
        num_gen=50_000, batch_size=128,
        device=torch.device("cuda"), desc="FID models: ", verbose=True, return_z=False):
...
"""
Computes the FID score for a generator models for a specific dataset
and a specific resolution
"""
...
def compute_kid(fdir1=None, fdir2=None, gen=None,
    ...
    # compute fid for a generator
        ...
        print(f"compute KID of a models with {dataset_name}-{dataset_res} statistics")
        # define the models if it is not specified
        model = build_feature_extractor(mode, device)
        ref_feats = get_reference_statistics(dataset_name, dataset_res,
                            mode=mode, seed=0, split=dataset_split, metric="KID")
        ...
        np_feats = get_model_features(gen, model, mode=mode,
            z_dim=z_dim, num_gen=num_gen, desc="KID models: ",
            batch_size=batch_size, device=device)
        score = kernel_distance(ref_feats, np_feats)
        return score
...
def compute_fid(fdir1=None, fdir2=None, gen=None,
    ...
    # compute fid for a generator
        ...
        if not verbose:
            print(f"compute FID of a models with {dataset_name}-{dataset_res} statistics")
