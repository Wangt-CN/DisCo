...
class ResnetBlock3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        # temporal conv
        self.tem_norm = torch.nn.GroupNorm(
            num_groups=groups_out, num_channels=out_channels, eps=eps, affine=True
        )
        self.tem_conv = nn.Conv1d(
            out_channels, out_channels, kernel_size=3, stride=1, padding=1
        )
        zero_module(self.tem_conv)
    ...
    def forward(self, input_tensor, temb):
        ...
        # temporal conv
        tem_residual = hidden_states
        hidden_states = self.tem_norm(hidden_states)
        hidden_states = self.nonlinearity(hidden_states)
        h, w = hidden_states.shape[-2:]
        hidden_states = rearrange(hidden_states, "b c f h w -> (b h w) c f")
        hidden_states = self.tem_conv(hidden_states)
        hidden_states = rearrange(hidden_states, "(b h w) c f -> b c f h w", h=h, w=w)
        hidden_states = hidden_states + tem_residual
