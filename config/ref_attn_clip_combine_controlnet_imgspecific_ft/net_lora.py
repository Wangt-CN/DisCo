...
class Net(nn.Module):
    def __init__(
        ...
        # for lora, load model here
        if args.pretrained_model_lora is not None:
            checkpoint = file2data(args.pretrained_model_lora, map_location='cpu')
            adaptively_load_state_dict(self, checkpoint['module'])
    ...
    def freeze_pretrained_part_in_ddpm(self):
        ...
        # # for lora, just freeze the unet
        # self.unet.requires_grad_(False)

        ### for stage-3 ft on specific image
        if self.args.freeze_pose:
            # for pose controlnet (net 0)
            for param_name, param in self.controlnet.named_parameters():
                if 'nets.0' in param_name:
                    param.requires_grad_(False)

        if self.args.freeze_background:
            for param_name, param in self.controlnet.named_parameters():
                if 'nets.1' in param_name:
                    param.requires_grad_(False)
