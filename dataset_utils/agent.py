...
class Agent():
    ...
    def eval_fn(
        ...
        with T.no_grad():
            ...
            for batch_idx, inputs in enumerate(eval_loader):
                ...
                # if len(inputs['img_key']) != self.args.local_eval_batch_size:
                #     continue
                inputs['embedding_objs'] = embedding_objs
                inputs['ref'] = refer_latents
                inputs = self.prepare_batch(inputs)
                outputs = self.forward_step(inputs)
                metric_and_loss = {
                    k: v
    ...
    def eval_fn_demo_batch(self, eval_loader, infer_filename, inner_collect_fn=None, use_tqdm=True, enc_dec_only=False, refer_latents=None, embedding_objs=None):
        self.model.eval()
        
        def tensor2pil_batch(images, resize_img=False, img_target_size=None):
            images = images.cpu().permute(0, 2, 3, 1).float().numpy()
            images = (images * 255).round().astype("uint8")
            if images.shape[0] == 1: #if images.shape[-1] == 1: #just for one image
        ...
        with T.no_grad():
            ...
            for batch_idx, inputs in enumerate(eval_loader):
                ...
                inputs = self.prepare_batch(inputs)
                outputs = self.forward_step(inputs)
                frames = tensor2pil_batch(outputs['logits_imgs'])
                for frame, name in zip(frames, inputs['img_key']):
                    path = os.path.join(infer_filename, name)
                    frame.save(path)        
        return True
    ...
    def eval_fn_demo(self, input_batch, enc_dec_only=False):
        ...
        with T.no_grad():
            ...
            output_image = tensor2pil(outputs['logits_imgs'], resize_img=self.args.pos_resize_img, img_target_size=None)
        return output_image
...
class Agent_LDM(Agent):
    ...
    def train_eval_by_iter(self, train_loader, eval_loader=None, use_tqdm=True, inner_collect_fn=None, refer_latents=None, embedding_objs=None):
        ...
        if self.args.eval_before_train and self.global_step == 0:
            ...
            if eval_loader:
                if self.args.eval_enc_dec_only:
                    ...
                    eval_meter, eval_time = self.eval_fn(eval_loader, inner_collect_fn=inner_collect_fn,
                                                        use_tqdm=use_tqdm, enc_dec_only=True, refer_latents=refer_latents, 
                ...
                eval_meter, eval_time = self.eval_fn(eval_loader, inner_collect_fn=inner_collect_fn,
                                                    use_tqdm=use_tqdm,refer_latents=refer_latents, embedding_objs=embedding_objs)
        ...
        while self.global_step < len(train_loader):
            ...
            if (self.global_step + 1) % self.args.eval_step == 0:
                ...
                if eval_loader:
                    eval_meter, eval_time = self.eval_fn(eval_loader, inner_collect_fn=inner_collect_fn,
                                                        use_tqdm=use_tqdm, train_eval_input=inputs,refer_latents=refer_latents, embedding_objs=embedding_objs) # sample a single round training sample to test whether over-fitting
        ...
        if self.global_step % self.args.eval_step != 0:
            ...
            if eval_loader:
                eval_meter, eval_time = self.eval_fn(eval_loader, inner_collect_fn=inner_collect_fn,
                                                    use_tqdm=use_tqdm, refer_latents=refer_latents, 
                                                        embedding_objs=embedding_objs)
                logger.info('[Rank %s] Valid Step: %d, Time: %s\n %s' %
                            (self.rank, self.global_step, eval_time, eval_meter.avg))
                val_meter_log = {**eval_meter.avg}
                ...
                if is_main_process():
                    self.update_metric_file(metric)
    def extract_latents(self, train_loader, use_tqdm=True, inner_collect_fn=None):
        # self.model.train(
        train_pbar = tqdm(total=len(train_loader), disable=not use_tqdm)
        self.model.train()
        self.global_step = 0
...
def filter_inputs(inputs):
    img_keys = inputs['img_key']#？
    camera=inputs['img_key'][0].split('_')[2]
    index = len([file for file in img_keys if camera in file])
    # return index==len(img_keys)-1
    return {key: value[:index] for key, value in inputs.items()}