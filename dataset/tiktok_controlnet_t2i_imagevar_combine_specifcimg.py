...
class BaseDataset(Dataset):
    def __init__(self, args, yaml_file, split='train', preprocesser=None):
        ...
        min_crop_scale = 0.5 if self.args.strong_aug_stage1 and split=='train' else 0.9 # for stronger augmentation (only use in train)
        ...
        if split == 'train':
            video_idx_start = 337 # only sample 1 image
            video_idx_end = 338
        else:
            video_idx_start = 337
            video_idx_end = 338
        ...
        for vid in range(video_idx_start, video_idx_end):
            ...
            if ft_img_num>0: # randomly sample n image as training sample
                ...
                frm_list = random.sample(list(range(1, num_frm)), ft_img_num)
                frm_list.sort()
                print('Random Sample Frames: {}'.format(frm_list))
            else:
                frm_list = list(range(1, num_frm))
            ...
            if ft_one_ref_image:
                if split == 'train':
                    ...
                    for fid in frm_list:
                        ...
                        ref_image_fname = self.image_path.format(vid, frm_list[0])
                        ref_mask_fname = self.ref_mask_path.format(vid, frm_list[0])  # only choose 1st frame as ref frame
                    ...
                    image_fname = self.image_path.format(vid, frm_list[0])
                    ref_image_fname = self.image_path.format(vid, frm_list[0])
                    ref_mask_fname = self.ref_mask_path.format(vid, frm_list[0])  # only choose 1st frame as ref frame

                    refpose_files = os.listdir(os.path.join(args.tiktok_data_root, '{:05d}/images/').format(vid - 2))
                    ...
                    for fid in range(1, refpose_image_files_len):
                        anno_pose_fname = self.anno_pose_path.format(vid - 2, fid % refpose_image_files_len)  # change to the next frame's pose
                        anno_fname = self.anno_path.format(vid - 2, fid % refpose_image_files_len)
                        self.image_paths_list.append(image_fname)
                        self.ref_image_paths_list.append(ref_image_fname)
                        self.anno_pose_list.append(anno_pose_fname)
                        self.anno_list.append(anno_fname)
                ...
                if split == 'train':
                    while len(frm_list) < 12:
                        frm_list *= 2 # enlarge the dataset to fix the dataloader bug
                    for fid in frm_list:
                        for ref_fid in frm_list:
                            image_fname = self.image_path.format(vid, fid)
                            ref_image_fname = self.image_path.format(vid, ref_fid)
                            anno_pose_fname = self.anno_pose_path.format(vid, fid)

                            anno_fname = self.anno_path.format(vid, fid)
                            ref_mask_fname = self.ref_mask_path.format(vid, ref_fid)  # only choose 1st frame as ref frame
                            self.image_paths_list.append(image_fname)
                            self.ref_image_paths_list.append(ref_image_fname)
                            self.anno_pose_list.append(anno_pose_fname)
                            self.anno_list.append(anno_fname)
                    ...
                    for fid in range(1, refpose_image_files_len):
                        ...
                        image_fname = self.image_path.format(vid, ref_fid) # just use ref image as the gt
                        ref_image_fname = self.image_path.format(vid, ref_fid) # randomly choose a image as ref
